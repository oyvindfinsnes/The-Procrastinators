...
if not os.path.isfile(os.path.join(current_dir, "src", "brain.brn")):
    try:
        subprocess.run(["python", os.path.join(current_dir, "src", "builder.py")], cwd=current_dir)
...
if env == supported_env["windows"]:
    try:
        subprocess.run(["python", "-m", "PyInstaller", os.path.join(current_dir, "src", "bot.py"), "--noconsole", "--name", supported_env["windows"], "--add-data", "src/brain.brn;."], cwd=current_dir)
...
# 3.x for the project, we want to compile into a single executable.
    ...
    try:
        subprocess.run(["python", "-m", "PyInstaller", os.path.join(current_dir, "src", "bot.py"), "--noconsole", "--name", supported_env["macos"], "--add-data", "src/brain.brn:."], cwd=current_dir)
...
# distribution uses, we want to compile into a single executable for simplicity.
    ...
    try:
        subprocess.run(["python", "-m", "PyInstaller", os.path.join(current_dir, "src", "bot.py"), "--name", supported_env["linux"], "--add-data", "src/brain.brn:."], cwd=current_dir)
